...
func startServer(hostAddr string, serviceType string, blockStoreAddr string) error {
	...
	if serviceType == "meta" {
		err := startMetaStore(blockStoreAddr, lis)
		if err != nil {
			return err
		}
	} else if serviceType == "block" {
		err := startBlockStore(lis)
		if err != nil {
			return err
		}
	} else if serviceType == "both" {
		err := startMetaAndBlockStore(blockStoreAddr, lis)
		if err != nil {
			return err
		}
	}
...
func startMetaStore(blockStoreAddr string, lis net.Listener) error {

	s := grpc.NewServer()
	surfstore.RegisterMetaStoreServer(s, surfstore.NewMetaStore(blockStoreAddr))
	log.Printf("meta server listening at %v", lis.Addr())
...
func startBlockStore(lis net.Listener) error {

	s := grpc.NewServer()
	surfstore.RegisterBlockStoreServer(s, surfstore.NewBlockStore())
	log.Printf("block server listening at %v", lis.Addr())
	if err := s.Serve(lis); err != nil {
		log.Fatalf("failed to serve: %v", err)
		return err
	}
...
func startMetaAndBlockStore(blockStoreAddr string, lis net.Listener) error {
	s := grpc.NewServer()
	surfstore.RegisterMetaStoreServer(s, surfstore.NewMetaStore(blockStoreAddr))
	surfstore.RegisterBlockStoreServer(s, surfstore.NewBlockStore())
	log.Printf("block and meta server listening at %v", lis.Addr())
	if err := s.Serve(lis); err != nil {
		log.Fatalf("failed to serve: %v", err)
		return err
	}
